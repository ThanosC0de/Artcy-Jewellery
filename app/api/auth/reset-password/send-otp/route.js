...
export async function POST(request) {
  try {
    ...
    if (!validatedData.success) {
      return response(
        false, 401, "Invalid or missing input field.", validatedData.error);
    }
    ...
    const otpSendStatus = await sendMail("Your email login verification code is.", email, otpEmail(otp));
